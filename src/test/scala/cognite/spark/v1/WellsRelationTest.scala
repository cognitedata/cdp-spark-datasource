...
class WellsRelationTest extends FlatSpec with Matchers with ParallelTestExecution with SparkTest {
  ...
  sourceDf.createOrReplaceTempView("sourceWells")
  ...
  "WellsRelation" should "read assets" taggedAs ReadTest in {
  ...
  it should "support pushdown filters on labels" taggedAs WriteTest in {
    ...
    retryWhile[Array[Row]](
      spark.sql(s"""select * from destinationWells
  ...
  it should "be possible to create assets" taggedAs WriteTest in {
    ...
    val df = spark.read
      ...
      .option("metricsPrefix", metricsPrefix)
      .load()
    df.createOrReplaceTempView("createWells")
    cleanupWells(assetsTestSource)
    try {
      spark
        .sql(s"""
      ...
      """.stripMargin)
        ...
        .write
        .insertInto("createWells")

      val assetsCreated = getNumberOfRowsCreated(metricsPrefix, "assets")
      assert(assetsCreated == 1)

      val Array(createdWell) = retryWhile[Array[Row]](
        spark.sql(s"select * from createWells where source = '$assetsTestSource'").collect(),
        rows => rows.length < 1)

      createdWell.getAs[String]("name") shouldBe "asset name"
      createdWell.getAs[Long]("dataSetId") shouldBe testDataSetId
      createdWell.getAs[String]("externalId") shouldBe externalId
    } finally {
      try {
  ...
  it should "be possible to copy assets from one tenant to another" taggedAs WriteTest in {
    ...
    try {
      ...
      """.stripMargin)
        ...
        .insertInto("assets")
      retryWhile[Array[Row]](
        spark.sql(s"select * from destinationWells where source = '$assetsTestSource'").collect(),
        rows => rows.length < 1)
    } finally {
      try {
        cleanupWells(assetsTestSource)
  ...
  it should "support upserts when using insertInto()" taggedAs WriteTest in {
    ...
    val destinationDf: DataFrame = spark.read
      ...
      .option("metricsPrefix", metricsPrefix)
      .load()
    destinationDf.createOrReplaceTempView("destinationWellsUpsert")
    ...
    try {
     ...
     """.stripMargin)
        .select(destinationDf.columns.map(col).toIndexedSeq: _*)
        .write
        .insertInto("destinationWellsUpsert")
      ...
      val assetsFromTestDf = retryWhile[Array[Row]](
        spark
          .sql(s"select * from destinationWellsUpsert where source = '$source' and description = 'foo'")
          .collect(),
        df => df.length != 100)
      ...
      spark
        ...
        .createOrReplaceTempView("destinationWellsUpsertCreated")
      val dfToUpdate = spark
        .sql(s"""
              ...
              |labels,
              |dataSetId
              |from destinationWellsUpsertCreated
      ...
      val descriptionsAfterUpsert = retryWhile[Array[Row]](
        spark
          .sql(
            s"select description from destinationWells where source = '$source' and description = 'bar'")
          .collect(),
        df => df.length < 200)
  ...
  it should "support some more partial updates" taggedAs WriteTest in {
    ...
    val destinationDf: DataFrame = spark.read
      ...
      .load()
    destinationDf.createOrReplaceTempView("destinationWellsMorePartial")
    ...
    try {
     ...
     """.stripMargin)
        .select(destinationDf.columns.map(col).toIndexedSeq: _*)
        .write
        .insertInto("destinationWellsMorePartial")

      // Check if post worked
      val assetsFromTestDf = retryWhile[Array[Row]](
        spark.sql(s"select * from destinationWellsMorePartial where source = '$source'").collect(),
        df => df.length < 100)
      assert(assetsFromTestDf.length == 100)
      ...
      val assetsWithNewNameDf = retryWhile[Array[Row]](
        {
          spark
            .sql(s"""
               ...
               |id from destinationWellsMorePartial
               |where source = '$source'
     """.stripMargin)
            .write
            .format("cognite.spark.v1")
          ...
          spark
            .sql(
              s"select * from destinationWellsMorePartial where source = '$source' and description = '$description'")
            .collect()
        },
        df => df.length < 100
      )
  ...
  it should "allow null ids on asset update" taggedAs WriteTest in {
    ...
    try {
     ...
     """.stripMargin)
        .select(destinationDf.columns.map(col).toIndexedSeq: _*)
        .write
        .insertInto("destinationWells")

      // Check if post worked
      val assetsFromTestDf = retryWhile[Array[Row]](
        spark
          .sql(s"select * from destinationWells where source = '$source' and description = 'foo'")
          .collect(),
        df => df.length < 100)
      assert(assetsFromTestDf.length == 100)
  ...
  it should "be possible to update name (using upsert) given only externalId" in {
    ...
    try {
     ...
     """.stripMargin)
          ...
          spark
            .sql(
              s"select name from destinationWells where source = '$source' and name = 'updated-name'")
            .collect()
        },
        df => df.length != 1
      )
  ...
  it should "be possible to update both name and externalId (using upsert) given id" in {
    ...
    try {
     ...
     """.stripMargin)
          ...
          spark
            .sql(
              s"select name from destinationWells where source = '$source' and name = 'updated-name' and externalId ='updated-$newExternalId'")
            .collect()
        },
        df => df.length != 1
      )
  ...
  it should "correctly have insert < read and upsert < read schema hierarchy" in {
    val assetInsert = WellIngestionInsertSchema(name = "test-asset")
    assetInsert.transformInto[WellsReadSchema].copy(id = 1234L)

    val assetUpsert = WellIngestionInsertSchema(name = Some("test-asset"), id = Some(1234L))
    assetUpsert
      .into[WellsReadSchema]
    ...
    try {
     ...
     """.stripMargin)
      ...
      val idsAfterInsert =
        retryWhile[Array[Row]](
          spark
            .sql(s"select id from destinationWells where source = '$source'")
            .collect(),
          df => df.length < 100)
    ...
    try {
     ...
     """.stripMargin)
      ...
      val idsAfterInsert =
        retryWhile[Array[Row]](
          spark
            .sql(s"select externalId from destinationWells where source = '$source'")
            .collect(),
          df => df.length < 100)
      assert(idsAfterInsert.length == 100)
  ...
  def cleanupWells(source: String): Unit =
    spark
      .sql(s"""select id from destinationWells where source = '$source'""")
      .write
      .format("cognite.spark.v1")
