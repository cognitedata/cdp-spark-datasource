...
final case class TestTypeBasic(
    a: Int,
    b: Double,
    c: Byte,
    d: Float,
    ...
    f: Seq[Long],
    s: String)
final case class TestTypeOption(
    a: Option[Int],
    b: Option[Double],
    c: Option[Byte],
    d: Option[Float],
    x: Option[Map[String, String]],
    g: Option[Long],
    f: Option[Seq[Option[Long]]],
    s: Option[String])
final case class TestTypeOptionalField(
    a: OptionalField[Int],
    b: OptionalField[Double],
    c: OptionalField[Byte])
...
class SparkSchemaHelperTest extends FlatSpec with ParallelTestExecution with Matchers {
  ...
  "SparkSchemaHelper structType" should "generate StructType from basic case class" in {
    structType[TestTypeBasic]() should be(
      StructType(Seq(
  ...
  it should "generate StructType from optional case class" in {
    structType[TestTypeOption]() should be(
      StructType(Seq(
  ...
  "SparkSchemaHelper fromRow" should "construct type from Row" in {
    val r = new GenericRowWithSchema(
      Array(1, 2.toDouble, 3.toByte, 4, Map("foo" -> "bar"), 5.toLong, Seq[Long](10), "foo"),
      structType[TestTypeBasic]())
    fromRow[TestTypeBasic](r) should be(
      TestTypeBasic(1, 2, 3, 4, Map("foo" -> "bar"), 5, Seq(10), "foo"))
  }

  "SparkSchemaHelper fromRow" should "construct type from Row doing implicit conversions" in {
    val r = new GenericRowWithSchema(
      Array(1, 2, 3.toByte, 4.toFloat, Map("foo" -> "bar"), 5.toLong, Seq[Long](10), "foo"),
      structType[TestTypeBasic]())
    fromRow[TestTypeBasic](r) should be(
      TestTypeBasic(1, 2, 3, 4, Map("foo" -> "bar"), 5, Seq(10), "foo"))
  }
  ...
  "SparkSchemaHelper fromRow" should "construct type from Row doing implicit conversions 2" in {
    val r = new GenericRowWithSchema(
      Array(
        1,
        new java.math.BigDecimal(2).pow(100),
        3.toByte,
    ...
    fromRow[TestTypeBasic](r) should be(
      TestTypeBasic(1, math.pow(2, 100), 3, 4, Map("foo" -> "bar"), 5, Seq(10), "foo"))
  }

  it should "construct optional type from Row of null" in {
    val r = new GenericRowWithSchema(
      Array(null, null, null, null, null, null, null, null),
      structType[TestTypeOption]())
    fromRow[TestTypeOption](r) should be(TestTypeOption(None, None, None, None, None, None, None, None))
  }

  it should "construct optional type from Row of map values that can be null" in {
    val r = new GenericRowWithSchema(
      Array(null, null, null, null, Map("foo" -> "row", "bar" -> "a"), null, null, null),
      structType[TestTypeOption]())
  ...
  it should "construct optional type from Row of seq values that can be null" in {
    val r = new GenericRowWithSchema(
      Array(null, null, null, null, null, null, Seq(20L, null), null),
      structType[TestTypeOption]())
    fromRow[TestTypeOption](r) should be(
  ...
  it should "ignore null in map" in {
    val x = new GenericRowWithSchema(
      Array(null, null, null, null, Map("foo" -> "row", "bar" -> null), null, null, null),
      structType[TestTypeOption]())
  ...
  it should "fail nicely on different type in map" in {
    val x = new GenericRowWithSchema(
      Array(null, null, null, null, Map("foo" -> "row", "bar" -> 1), null, null, null),
      structType[TestTypeOption]())
  ...
  it should "fail nicely on unexpected NULL in int" in {
    val x = new GenericRowWithSchema(
      Array(null, 2.toDouble, 3.toByte, 4.toFloat, Map("foo" -> "bar"), 5.toLong, Seq[Long](10), "foo"),
      structType[TestTypeBasic]())
    val ex = intercept[CdfSparkIllegalArgumentException] { fromRow[TestTypeBasic](x) }
  ...
  it should "fail nicely on unexpected NULL in string" in {
    val x = new GenericRowWithSchema(
      Array(1, 2.toDouble, 3.toByte, 4.toFloat, Map("foo" -> "bar"), 5.toLong, Seq[Long](10), null),
      structType[TestTypeBasic]())
    val ex = intercept[CdfSparkIllegalArgumentException] { fromRow[TestTypeBasic](x) }
  ...
  it should "fail nicely on unexpected NULL in map" in {
    val x = new GenericRowWithSchema(
      Array(1, 2.toDouble, 3.toByte, 4.toFloat, null, 5.toLong, Seq[Long](10), "foo"),
      structType[TestTypeBasic]())
    val ex = intercept[CdfSparkIllegalArgumentException] { fromRow[TestTypeBasic](x) }
