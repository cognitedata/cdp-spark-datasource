...
class DataPointsRelationTest extends FlatSpec with Matchers with SparkTest {
  ...
  it should "apply limit to each partition" taggedAs(ReadTest) in {
    val df = spark.read.format("com.cognite.spark.datasource")
      ...
      .option("partitions", "2")
      .load()
      .where(s"timestamp >= 1509528850000 and timestamp <= 1557485862500 and name = $valhallTimeSeries")
    assert(df.count() == 200)
  ...
  it should "support aggregations" taggedAs(ReadTest) in {
    val df1 = spark.read.format("com.cognite.spark.datasource")
      ...
      .option("partitions", "1")
      .load()
      .where(s"timestamp >= 1509490000000 and timestamp <= 1510358400000 and aggregation = 'min' and granularity = '1d' and name = $valhallTimeSeries")
    assert(df1.count() == 10)
    val df1Partitions = spark.read.format("com.cognite.spark.datasource")
      .option("apiKey", readApiKey)
      .option("type", "datapoints")
      .load()
      .where(s"timestamp >= 1509490000000 and timestamp <= 1510358400000 and aggregation = 'min' and granularity = '1d' and name = $valhallTimeSeries")
    ...
    val df3 = spark.read.format("com.cognite.spark.datasource")
      ...
      .option("type", "datapoints")
      .load()
      .where(s"timestamp >= 1514592000000 and timestamp <= 1540512000000 and aggregation = 'avg' and granularity = '60d' and name = $valhallTimeSeries")
  ...
  it should "shift non-aligned aggregates to correct timestamps" taggedAs ReadTest in {
    val df1 = spark.read.format("com.cognite.spark.datasource")
      ...
      .load()
      .sort("timestamp")
      .where(s"timestamp >= 1509490001000 and timestamp <= 1510358400000 and aggregation = 'min' and granularity = '1d' and name = $valhallTimeSeries")
  ...
  it should "be possible to specify multiple aggregation types in one query" taggedAs(ReadTest) in {
    val df = spark.read.format("com.cognite.spark.datasource")
      ...
      .option("partitions", "1")
      .load()
      .where(s"timestamp >= 1508889600000 and timestamp <= 1511481600000 and aggregation in ('min', 'avg', 'max') and granularity = '30d' and name = $valhallTimeSeries")
    assert(df.count() == 3)
