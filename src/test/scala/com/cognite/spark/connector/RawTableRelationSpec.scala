...
class RawTableRelationSpec extends FlatSpec with Matchers with DataFrameSuiteBase {
  ...
  "A RawTableRelation" should "allow data columns named key, _key etc. but rename them to _key, __key etc." in {
    ...
    val dfWithoutKeySchema = StructType(Seq(
      StructField("notKey", StringType, false),
      StructField("value", IntegerType, false)))
    val dfWithoutKey = Seq(
      ("key1", """{ "notKey": "k1", "value": 1 }"""),
      ("key2", """{ "notKey": "k2", "value": 2 }""")
    ).toDF("key", "columns")

    val dfWithKeySchema = StructType(Seq(
