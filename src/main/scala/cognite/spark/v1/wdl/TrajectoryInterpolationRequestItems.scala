...
case class TrajectoryInterpolationRequestItems(
  items: Seq[TrajectoryInterpolationRequest],
  /* Ignore wellbores that don't have a definitive trajectory. */
  ignoreMissingTrajectories: Option[Boolean] = None,
  /* Ignore measured depths that are outside the trajectory range. */
  ignoreUnknownMeasuredDepths: Option[Boolean] = None
)
