...
case class WellItems(
  items: Seq[Well],
  /* Number of wells that matches the query. This is only set if `aggregates` in the request body contains `\"count\"`. */
  wellsCount: Option[Int] = None,
  /* Number of wellbores that matches the query. This is only set if `aggregates` in the request body contains `\"count\"`. */
