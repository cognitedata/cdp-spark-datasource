...
import cognite.spark.v1.wdl.WellDataAvailabilityFilterEnum._

  /**
   * This filter will only return wells that have at least one element of any type from the list provided
   */
case class WellDataAvailabilityFilter(
  containsAny: Seq[WellDataAvailabilityFilterEnum]
)
