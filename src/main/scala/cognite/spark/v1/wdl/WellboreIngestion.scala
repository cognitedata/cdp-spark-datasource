...
case class WellboreIngestion(
  ...
  /* The asset external ID of the well this wellbore is associated with. */
  wellAssetExternalId: String,
  /* Parent wellbore if it exists. */
  parentWellboreAssetExternalId: Option[String] = None,
  /* `uniqueWellboreIdentifier` (also called UBI) is a guaranteed unique identifier. All wellbores that share the same `matchingId` must also share the same `uniqueWellboreIdentifier`, or have it set to `null`.  */
  ...
  kickoffMeasuredDepth: Option[WellboreKickoffMeasuredDepth] = None
)
