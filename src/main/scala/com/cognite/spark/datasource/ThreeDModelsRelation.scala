...
class ThreeDModelsRelation(config: RelationConfig)(@transient val sqlContext: SQLContext)
  ...
  @transient lazy private val modelsRead =
    UserMetricsSystem.counter(s"${config.metricsPrefix}3dmodels.read")
